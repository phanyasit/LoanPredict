...
#Load Model
pickle_in = open('RandomForest-GridSearchCV.pkl', 'rb')
...
if st.button("ทำนาย"):
    ...
    #st.write("รถของคุณคือ : ", Gender, Occupation, car_type[0], car_brand[0], car_model[0])
    st.write("% LTV :", loan_ratio)

    
    
    # 'Principal_Approve_Amount', 'Repayment_Tenor_Month_Number', 'Gender', 'Occupation_Description',
    ...
    # prediction = classifier.predict([[51000,36,0,10,10000,1,5,135,2018,64500,28,5]])
    # prediction = classifier.predict([[51000, installment_term, 0, 10,monthly_income, 1, 5,135,2018,64500,41,10]])
    prediction = classifier.predict([[Principal_Approve_Amount, Repayment_Tenor_Month_Number, Gender, Occupation, monthly_income, car_type[0], car_brand[0], car_model[0], car_year, Evaluation_Amount, age, car_age]])
    if prediction == 'PL':
            st.balloons()
            st.write("")
